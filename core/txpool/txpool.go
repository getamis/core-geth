...
func NewTxPool(config Config, chainconfig ctypes.ChainConfigurator, chain blockChain) *TxPool {
...
func (pool *TxPool) runReorg(done chan struct{}, reset *txpoolResetRequest, dirtyAccounts *accountSet, events map[common.Address]*sortedMap) {
	...
	if reset != nil {
		...
		if reset.newHead != nil && pool.chainconfig.IsEnabled(pool.chainconfig.GetEIP1559Transition, new(big.Int).Add(reset.newHead.Number, big.NewInt(1))) {
