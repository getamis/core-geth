...
func writeAncientBlock(op ethdb.AncientWriteOperator, block *types.Block, header *types.Header, receipts []*types.ReceiptForStorage, td *big.Int, transferLogs []*types.TransferLog) error {
	num := block.NumberU64()
	...
	if err := op.AppendRaw(freezerTransferLogTable, num, transferLogBlob); err != nil {
